...
def db_get_pending_requests(session_code):
    conn = sqlite3.connect(DB)
    rows = conn.execute("SELECT student_id FROM join_requests WHERE session_code=? AND status='pending'",
...
class InterviewAudio:
    ...
    SAMPLE_RATE   = 48000   # Hz  — studio quality
    CHANNELS      = 1       # mono (works on all webcams)
    DTYPE         = "int16"
    CHUNK_MS      = 30      # milliseconds per chunk
    CHUNK_FRAMES  = int(SAMPLE_RATE * CHUNK_MS / 1000)
    GAIN          = 1.6     # slight volume boost
        ...
        try:
            with sd.InputStream(samplerate=self.SAMPLE_RATE,
                ...
                while self._running:
                    ...
                    try:
                        _requests.post(url, data=raw, timeout=0.5,
                ...
                while self._running:
                    try:
                        ...
                        if r.status_code == 200 and r.content:
                            ...
                            if len(arr) > 0:
                                stream.write(arr.reshape(-1, self.CHANNELS))
                        else:
                            # No data yet — write silence to keep stream open
                            silence = np.zeros(
                                (self.CHUNK_FRAMES, self.CHANNELS), dtype=np.int16)
...
def start_network_server(port=6000):
    ...
    MAX_AUDIO_CHUNKS = 8      # keep at most ~240 ms buffered
...
class MultiStudentProctorWindow:
    ...
    def _add_student_tile(self, sid):
        ...
        # ── Camera feed — taller for better visibility ──
        cam_lbl = tk.Label(card, bg="#0b0b13",
                           text="Waiting for camera…", fg="#3a3a5a",
                           font=("Helvetica",8))
        cam_lbl.pack(fill="both", expand=True, padx=4, pady=(2,0))
        cam_lbl.configure(height=12)   # ~150px
    ...
    def _poll_cameras(self):
        ...
        # 1. Check for new accepted students
        ...
        # 3. Auto-refresh violation log — throttled to STATS_MS
